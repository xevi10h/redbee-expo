-- =====================================================
-- VIDEO ANALYTICS SYSTEM - DATABASE MIGRATIONS (CORREGIDA)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS video_views (
    ...
    viewer_id UUID REFERENCES profiles(id) ON DELETE SET NULL, -- NULL para vistas anónimas
...
CREATE INDEX IF NOT EXISTS idx_video_views_viewer_country ON video_views(viewer_country);
CREATE INDEX IF NOT EXISTS idx_video_views_viewed_at ON video_views(viewed_at);
CREATE INDEX IF NOT EXISTS idx_video_views_video_created_at ON video_views(video_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_video_daily_metrics_video_date ON video_daily_metrics(video_id, date);
...
CREATE INDEX IF NOT EXISTS idx_video_hourly_metrics_video_date_hour ON video_hourly_metrics(video_id, date, hour);

-- 5. ACTUALIZAR TABLA DE REPORTES (adaptada al esquema existente)
-- Agregar campos necesarios para analíticas de reportes
DO $$ 
BEGIN
    -- Agregar columnas si no existen (la tabla reports ya existe)
    ...
    ALTER TABLE reports ADD COLUMN IF NOT EXISTS automated_action VARCHAR(50) DEFAULT 'none';
    
    -- Actualizar la referencia a reported_video_id si existe
    UPDATE reports SET video_id = reported_video_id WHERE reported_video_id IS NOT NULL AND video_id IS NULL;
END $$;
...
BEGIN
    ...
    SELECT 
        target_video_id,
        target_date,
        EXTRACT(hour FROM viewed_at)::INTEGER as hour,
        COUNT(*) as views_count,
        COUNT(DISTINCT COALESCE(viewer_id::text, viewer_ip_hash)) as unique_views_count
    FROM video_views
    WHERE video_id = target_video_id 
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'video_views_metrics_trigger') THEN
...
DROP POLICY IF EXISTS "video_analytics_owner_only" ON video_views;
DROP POLICY IF EXISTS "video_shares_owner_only" ON video_shares;
DROP POLICY IF EXISTS "video_daily_metrics_owner_only" ON video_daily_metrics;
DROP POLICY IF EXISTS "video_hourly_metrics_owner_only" ON video_hourly_metrics;
DROP POLICY IF EXISTS "anyone_can_create_views" ON video_views;
DROP POLICY IF EXISTS "anyone_can_create_shares" ON video_shares;
...
CREATE INDEX IF NOT EXISTS idx_video_views_analytics 
ON video_views(video_id, created_at, is_premium_viewer, is_follower);

CREATE INDEX IF NOT EXISTS idx_video_views_geography 
ON video_views(video_id, viewer_country, viewer_city);

CREATE INDEX IF NOT EXISTS idx_video_views_engagement 
