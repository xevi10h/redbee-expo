...
export const CommentsDetailModal: React.FC<CommentsDetailModalProps> = ({
	isVisible,
	videoId,
	videoTitle,
	onClose,
	...
	const loadAllComments = useCallback(async () => {
		try {
			...
			const response = await AnalyticsService.getRecentComments(videoId, 1000);
	...
	const handleSearch = useCallback(
		async (query: string) => {
			...
			setIsSearching(true);
			try {
				const response = await AnalyticsService.searchCommentsByUsername(
					videoId,
					query,
				);
				if (response.success) {
